...
- [x] Task 1: Commit 1 — mover as classes de `configuration/cache/*` (AC: #1)
  - [x] Criar módulo Maven `scos-foundation-cache`, pacote raiz `br.com.sawcunhaos.foundation.cache`, dependendo só de `core` + `spring-boot-starter-cache` + `spring-boot-starter-data-redis` (+ 3 dependências adicionais inevitáveis, ver Debug Log)
