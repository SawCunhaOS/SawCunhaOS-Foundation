...
package br.com.sawcunhaos.foundation.exception.listener;
...
/**
 * Story 1.15 (AC #1): logs, once at startup, how many {@code @CPF}/{@code @CNPJ}/
 * {@code @TaxIdentifier}/{@code @ZipCode} field usages were found on the consuming application's
 * own classpath. This is observability only — Jakarta Bean Validation already does its own
...
public class ValidationAnnotationCountListener implements ApplicationListener<ApplicationReadyEvent> {
    ...
    public void onApplicationEvent(ApplicationReadyEvent event) {
        ...
        try {
            ...
            log.warn("ValidationAnnotationCountListener: could not resolve the application's base package(s), skipping the scan", e);
            return;
        }
        int count = countAnnotatedElements(basePackages);
        log.info("ValidationAnnotationCountListener: found {} @CPF/@CNPJ/@TaxIdentifier/@ZipCode field usage(s) on the classpath", count);
    }
            ...
            try {
                ...
                for (Field field : clazz.getDeclaredFields()) {
                    if (field.isAnnotationPresent(CPF.class)
                            || field.isAnnotationPresent(CNPJ.class)
                            || field.isAnnotationPresent(TaxIdentifier.class)
                            || field.isAnnotationPresent(ZipCode.class)) {
                        count++;
                    }
                }
            } catch (ClassNotFoundException | LinkageError e) {
                log.warn("ValidationAnnotationCountListener: skipped unloadable class {}", className, e);
            }
        }
        return count;
    }
