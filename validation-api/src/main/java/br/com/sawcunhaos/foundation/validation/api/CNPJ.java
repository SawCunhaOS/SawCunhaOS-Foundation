 ...
 * ({@code CnpjValidator}) lives in {@code scos-foundation-validation}, which depends on this
...
public @interface CNPJ {
    ...
    /**
     * Error code carried on the violation when the annotated value is not a valid CNPJ — not a
     ...
     * with a literal message (e.g. {@code @CNPJ(message = "...")}) bypasses that resolution — the
     * violation surfaces the literal text instead of the translated code.
     */
    String message() default "SCOS-008";

    /**
     * Jakarta Bean Validation group(s) this constraint belongs to. Not used by
     * {@code CnpjValidator} — present only to satisfy the {@code @Constraint} contract.
     */
    Class<?>[] groups() default {};

    /**
     * Client-supplied metadata attached to a violation. Not used by {@code CnpjValidator} —
